...
def MAN_Res_Bottle_Test():
    return _mna_net(
        "MAN_Res_Bottle_Test", bottleneck_block, [2, 2, 2, 2, 2], residual=True
    )
...
def R_MG16():
    return _mna_net(
        "R_MG16", base_block, [3, 4, 4, 4, 4], progressive=False, residual=True
    )


def PMG16():
    return _mna_net(
        "PMG16", base_block, [2, 3, 3, 3, 3], progressive=True, residual=False
    )


def R_PMG16():
    return _mna_net(
        "R_PMG16", base_block, [1, 2, 2, 2, 1], progressive=True, residual=True
    )
